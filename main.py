...
red_cards = []  # Will hold the red team's cards
blue_cards = []  # Will hold the blue team's cards
dragging_card = None  # Will hold the card currently being dragged, if any
current_team = None  # Will indicate which team's turn it is
player_team = None  # Will indicate which team the player is on
ai_team = None
...
def initialize_board():
    ...
    board.clear()
    board.extend([[None for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)])
...
async def main():
    global dragging_card, current_team, player_team, selection_mode, selected_card_index, grid_row, grid_col, board, red_cards, blue_cards, ai_team, current_volume
    show_title_screen()
    show_team_selection_screen()
    if current_team is None:
        current_team = player_team
    print(f"Initial team settings: current_team = {current_team}, player_team = {player_team}")
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()
                    ...
                    if current_team == player_team:  # Ensure it's the player's turn
                        for card in red_cards if player_team == 'red' else blue_cards:
                            if card.rect.collidepoint(event.pos):
                                ...
                                print(f"Dragging card: {dragging_card}")
                                break
                ...
                if selection_mode == DECK_SELECTION:
                    ...
                    if event.key == pygame.K_RETURN or event.key == pygame.K_SPACE:
                        if board[grid_row][grid_col] is None:
                            ...
                            print("Switching to DECK_SELECTION")
